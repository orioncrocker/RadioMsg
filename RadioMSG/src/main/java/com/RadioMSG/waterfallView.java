...
//public class waterfallView extends android.support.v7.widget.AppCompatImageView {
public class waterfallView extends AppCompatImageView {
	private int xDim = 0;
	private int yDim = 100;
	private int[] Buffer;
	private double Amplitude;
	//public double waterfallDynamic = -6.0;
	...
	public waterfallView(Context context, AttributeSet attrs) {
		...
		start = (int) (500.0 * 1488.0 / Modem.RSID_SAMPLE_RATE);
		end = 1 + (int) (3000.0 * 1488.0 / Modem.RSID_SAMPLE_RATE);
		xDim = end - start;
		Buffer = new int[xDim * yDim];
		maxvalue = RadioMSG.mysp.getFloat("WFMAXVALUE", (float) 11.0);
		myBmPaint = new Paint();
	...
	public void onDraw(Canvas canvas) {
		...
		if (Modem.newAmplReady)
		{
			...
			{
				//Amplitude = Math.log(Modem.WaterfallAmpl[i]) - 6.0f;
				Amplitude = Math.log(Modem.WaterfallAmpl[i]) + 10.0f;
				//Amplitude = Math.log(Modem.WaterfallAmpl[i] * RadioMSG.waterfallDynamic) -6; //old gain of -6
				...
				blue = (int) (1000 * RadioMSG.waterfallDynamic * (maxvalue - Amplitude) / maxvalue);
