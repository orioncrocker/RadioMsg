...
public class RMsgCheckSum {
	...
	public static String Crc16(String intext) {
		...
		//Remap the crc characters to lower case characters ONLY
		// by shifting the code up by 49 decimal when encoding
		//Benefits are faster transfer (varicodes and CCIR476 does not require shifting/unshifting)
		encrypted += Integer.toHexString(crc1);
		encrypted = encrypted.substring(encrypted.length()-4);
		String lowerCaseCrc = "";
        char oneChar;
		for (int i=0; i < 4; i++) {
            oneChar = encrypted.charAt(i);
            if (oneChar < 'a') {
                oneChar += 55;
            }
            lowerCaseCrc += Character.toString(oneChar);
		}
	...
	//The drawback of the time based passwords is the need for time synchronisation between the client and server, which can be initiated by the client using the "Time Sync" function.
	public static boolean checkCrcWithPasswordAndTime(String messageLessCrc, String password, String mFilename, String crcRxValue) {
		...
		if (password.equals("")) {
			...
			for (int i = -1; i < 4; i++) {
				...
				byte[] md5 = org.apache.commons.codec.digest.DigestUtils.md5(messageLessCrc + password + testTimeSlot.toString());
