...
#define RSID_SAMPLE_RATE 8000.0

//RadioMSG test double speed
//#define RSID_FFT_SAMPLES 	512
//#define RSID_FFT_SIZE		1024
//300 ok in sensitivity

//8000Hz SR
//#define RSID_FFT_SAMPLES 	512
//#define RSID_FFT_SIZE		1024
#define RSID_FFT_SAMPLES 	372
#define RSID_FFT_SIZE		744
...
//#define RSID_SYMLEN		(1024.0 / RSID_SAMPLE_RATE) // 0.09288 // duration of each rsid symbol
#define RSID_SYMLEN		(744.0 / RSID_SAMPLE_RATE) // 0.09288 // duration of each rsid symbol
...
//8000Hz SR follow kiss_fft.h type definition
//typedef double rs_fft_type;
typedef kiss_fft_scalar rs_fft_type;
...
//Android using old fft routines	rs_cpx_type		aFFTReal[RSID_ARRAY_SIZE];
    ...
    //kiss_fft_cpx fftResult[RSID_FFT_SIZE];
    kiss_fft_cpx fftResult[RSID_ARRAY_SIZE];

//Android	g_fft<rs_fft_type>		*rsfft;
	Cfft	*rsfft;
	//8000Hz SR, use kissfft for non power of 2 size
    kiss_fftr_cfg cfg;
...
//	void	receive(const float* buf, size_t len);
	...
	//void	receive(const short* buf, size_t len, bool doSearch);
	void 	send(bool postidle);
    //RadioMSG added
	void 	sendThis(int code);
	//Android added
