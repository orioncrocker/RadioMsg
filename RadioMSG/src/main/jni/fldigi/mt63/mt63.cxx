...
//Android not yet		fprintf(stderr, "mt63_rxprocess: buffer error\n");
		return -1;
	}
...
//Android		guard_lock dsp_lock(&mt63_mutex);
		...
		if (err)
//Android not yet			fprintf(stderr, "mt63_txinit: init failed\n");
		flush = Tx->DataInterleave;
...
mt63::mt63 (int mt63_mode) : modem()
{
	...
	progdefaults.mt63_8bit		  	= getPreferenceB("MT638BIT", true);
	progdefaults.mt63_rx_integration= getPreferenceB("MT63INTEGRATION", false);
	progdefaults.mt63_usetones	  	= getPreferenceB("MT63USETONES", false);
	progdefaults.mt63_twotones	  	= getPreferenceB("MT63TWOTONES", true);
	progdefaults.mt63_tone_duration	= getPreferenceI("MT63TONEDURATION", 4);
	//Reversed logic for mt63_at500 (GUI says: "Allow Manual Tuning")
	progdefaults.mt63_at500		  	= !getPreferenceB("MT63AT500", true);

	progdefaults.mt63_tone_duration = (int) clamp(progdefaults.mt63_tone_duration, 1.1, 10.1);
...
{
    //RadioMSG remove fixed lower frequency
