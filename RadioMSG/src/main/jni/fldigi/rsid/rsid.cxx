...
//Android Could not find the definition anywhere (used arbitrary numbers for NONE and NONE2)
...
cRsId::cRsId()
{
	...
	progdefaults.rsid_min_bw = getPreferenceI("RSIDMINBW", 200);
...
//Android rsfft = new g_fft<rs_fft_type>(RSID_ARRAY_SIZE);
	//Use kissfft for non power of 2 size
	// rsfft = new Cfft(RSID_FFT_SIZE);
    cfg = kiss_fftr_alloc(RSID_FFT_SIZE * 2, 0, NULL, NULL);
...
//Android add flag for rx RSID as we want to keep the FFT processing for the waterfall
//void cRsId::receive(const float* buf, size_t len)
//void cRsId::receive(const short* shortbuf, size_t len, bool doSearch)
void cRsId::receive(const float* buf, size_t len, bool doSearch)
{
	//float mybuf[40000]; //Getting memory leaks with New/Delete. Using 5 seconds of audio max.
    //float* buf = mybuf;
    //int sizeOfFloatBuffer = sizeof(mybuf) / sizeof(float);

	if (len == 0) return; //Nothing to do

	//Sampling at 8000Hz
	//Convert array from ints to floats
	//short const* sb = shortbuf;
    //test ML issue float* buf = new float[len];
	//for (int z=0; z < len && z < sizeOfFloatBuffer; z++) {
	//    buf[z] = (float) shortbuf[z];
	    //*buf++ = (*sb++);
	//}
    ...
    //****Warning may be used below for memmove??  double src_ratio = RSID_SAMPLE_RATE / active_modem->get_samplerate();

	if (rsid_secondary_time_out > 0) {
    //Android fixed for now		rsid_secondary_time_out -= (int)(len / src_ratio);
		rsid_secondary_time_out -= (int)(len);
		if (rsid_secondary_time_out <= 0) {
    //Android not yet			LOG_INFO("%s", "Secondary RsID timed out");
			reset();
		}
	}
    ...
    */
    ...
    //Android Just copy into buffer since it is already re-sampled to 11025Hz (this allows the re-use
    //   of the resampled data for other modems like mfsk and Thor rather than re-sample twice)
...
void cRsId::search(bool doSearch)
{
    if (progdefaults.rsidWideSearch) {
        ...
        //Added variable search bandwidth as per Fldigi 4.2.04.11
...
//Android aFFTcmplx[i] = cmplx(aInputSamples[i] * fftwindow[i], 0);
		aFFTReal[i] = (rs_fft_type)aInputSamples[i] * fftwindow[i];
	} else {
		for (int i = 0; i < RSID_ARRAY_SIZE; i++)
//Android			aFFTReal[i] = cmplx(aInputSamples[i], 0);
		aFFTReal[i] = (rs_fft_type)aInputSamples[i];
	}
//Bug in Android
//memset(aFFTReal + RSID_FFT_SIZE, 0, RSID_FFT_SIZE * sizeof(rs_fft_type));
//Using kissfft as we use a non power of 2 size
	//rsfft->rdft(aFFTReal);
    kiss_fftr(cfg, aFFTReal, fftResult);
...
*/
    ...
    //Calculate the total amplitude for audio level indicator
    audioLevel = 0.0f;
    for (int i = 0; i < RSID_FFT_SIZE; i++) {
        audioLevel += aFFTAmpl[i];
    }
...
//Android	ENSURE_THREAD(TRX_TID);
	...
	rsidfreq = (iBin + RSID_NSYMBOLS - 0.5) * RSID_SAMPLE_RATE / (RSID_FFT_SIZE * 2);
	...
	if (mbin == NUM_MODES) {
		...
		if (n < tblsize) // RSID known but unimplemented
			...
			snprintf(msg, sizeof(msg), "\nRSID: {code %d unknown}", iSymbol);
//Android		put_status(msg, 4.0);
//Android		LOG_VERBOSE("%s", msg);
	ii = 0;
	while (ii < sizeof(msg) && msg[ii] != '}') {
		put_rx_char(msg[ii]);
		ii++;
	}
	//Last  character of "{abc}" above
	put_rx_char('}');
		return;
	}
...
*/
	...
	//Moved from below
	//change_CModem(p_rsid[n].mode, rsidfreq);
...
// Try secondary table first
	...
	if (rmode == RSID_NONE) {
//Android not yet		LOG_DEBUG("%s mode is not assigned an RSID", mode_info[mode].sname);
		return false;
	}
	return true;
}
...
// transmit 1 symbol periods of silence at beginning of rsid
//VK2ETA remove initial silence to keep VOX ON after pre-transmission tone
...
// transmit sequence of 15 symbols (tones)
	...
	fr = 1.0 * active_modem->get_txfreq() - (RSID_SAMPLE_RATE * 7 / 744);
	phase = 0.0;
	...
	for (int i = 0; i < 15; i++) {
		...
		freq = fr + iTone * RSID_SAMPLE_RATE / 744;
...
//Send a specific RSID code independant of current
//mode for signalization purposes
void cRsId::sendThis(int code)
{

    rmode = (unsigned short) code;
