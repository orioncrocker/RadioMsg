...
#include <stdlib.h>
#include <stdio.h>
//Android #include <iomanip>
//Android #include <iostream>
//Android #include <complex>
#include "complex.h"
...
//Android #include "viewpsk.h"
//Android #include "pskeval.h"
#include "modem.h"
//Android #include "Viewer.h"
...
//Android void psk::tx_init(SoundBase *sc)
void psk::tx_init()
{
//Android 	scard = sc;
...
void psk::rx_init()
{
	...
	bitclk = 0;
	freqerr = 0.0;
//Android 	if (mailserver && progdefaults.PSKmailSweetSpot) sigsearch = SIGSEARCH;
//Android	else sigsearch = 0;
//Android todo fix this (maybe)	put_MODEstatus(mode);
	resetSN_IMD();
	imdValid = false;
	afcmetric = 0.0;
...
void psk::restart()
{
//Android	if ((mode >= MODE_PSK31 && mode <= MODE_PSK125) ||
//Android		(mode >= MODE_QPSK31 && mode <= MODE_QPSK125))
//Android		pskviewer->restart(mode);
//JD check this? Android?	evalpsk->setbw(sc_bw);
}
...
void psk::init()
{
	restart();
	modem::init();
//Android 	set_scope_mode(Digiscope::PHASE);
	initSN_IMD();
	snratio = 1.0;
	imdratio = 0.001;
	rx_init();
...
//Android	if (pskviewer) delete pskviewer;
//Android	if (evalpsk) delete evalpsk;
...
//Android psk::psk(trx_mode pskmode) : modem()
psk::psk(int pskmode) : modem()
{

	//progdefaults.pskpilot = true;
	//Android Added access to progdefaults(preferences) in the Java side
	progdefaults.pskpilot  = getPreferenceB("8PSKPILOT", true);
	progdefaults.pilot_power = getPreferenceD("8PSKPILOTPOWER", -30.0f);
	//Limit pilot_power
	if (progdefaults.pilot_power > -20.0f) progdefaults.pilot_power = -20.0f;
	if (progdefaults.pilot_power < -60.0f) progdefaults.pilot_power = -60.0f;
...
//Android			samplerate = 16000;
	...
	// 8psk modes with FEC
		case MODE_8PSK125F:  // 125 baud | 188 bits/sec @ 1/2 Rate FEC
//Android			symbollen = 128;
			symbollen = 64;
...
//Android			samplerate = 16000;
			...
			cap |= CAP_REV;
			break;
		case MODE_8PSK1000F: // 1000 baud | 2000 bits/sec @ 2/3 rate FEC
 //Android			symbollen = 16;
			symbollen = 8;
			idepth = 512;
			flushlength = 56;
//Android			samplerate = 16000;
		...
		case MODE_8PSK1200F: // 1200 baud | 2400 bits/sec @ 2/3 rate FEC
			...
			PSKviterbi = true;
			break;
		case MODE_8PSK500FX2: // 500 baud | 2000 bits/sec @ 2/3 OR 1500 bits/sec @ 1/2 rate FEC
//Android			symbollen = 32;
			symbollen = 16;
			idepth = 640; // 426 milliseconds
//Android			flushlength = 62;
...
//Android			samplerate = 16000;
			samplerate = 8000;
			separation = 1.8; //Needed due to inter-carrier QRM at 1.4
			numcarriers = 2;
			dcdbits =1024;
//			_puncturing = true;
			idepth = 512;
...
//Android quick fix as it is ALWAYS Zero
//	switch (progdefaults.PSK_filter) {
	switch (0) {
		...
		case 4:
			...
			//Android missing break?
			break;
		case 0:
		default :
			//Brought CPU load reduction from AndPskmail Java modems
			//Dependant on slowcpu option.
			if (progdefaults.slowcpu) {
				// creates fir1c matched sin(x)/x filter w blackman
				wsincfilt32(fir1c, 1.0 / symbollen, true);
				// creates fir2c matched sin(x)/x filter w blackman
				wsincfilt32(fir2c, 1.0 / 16.0, true);
				for (int i = 0; i < numcarriers; i++) {
					fir1[i]->init(FIRLEN/2, symbollen > 15 ? symbollen / 16 : 1, fir1c, fir1c);
					fir2[i]->init(FIRLEN/2, 1, fir2c, fir2c);
				}
			} else {
				// creates fir1c matched sin(x)/x filter w blackman
	...
	if (_disablefec) {
		...
		dec->setchunksize(4);
		dec2 = new viterbi(K11, K11_POLY1, K11_POLY2);
...
void psk::searchDown()
{
//Android: review when have selectable afc
/*
...
void psk::vestigial_afc() {
	...
	double avg = 0;
	int i = 0;
	//Android old complex.h    for (i = 0; i < 11; i++) avg += abs(sfft_bins[i]);
	for (i = 0; i < 11; i++) avg += sqrt(sfft_bins[i].norm());
	avg /= 11.0;
	std::setprecision(2);
	std::setw(5);
	//Android old complex.h    	for (i = 0; i < 11; i++) if (abs(sfft_bins[i]) > 2.0*avg) break;
	for (i = 0; i < 11; i++) if (sqrt(sfft_bins[i].norm()) > 2.0*avg) break;
	if (i < 11) {
//		std::cout	<< "bin: " << i
...
void psk::phaseafc()
{
	...
	//Methods returns void, so can't "return vestigial_afc();"
	if (vestigial) {
		vestigial_afc();
		return;
	}
	...
	if (afcmetric < 0.05 ||
		mode == MODE_PSK500 ||
//Android		mode == MODE_QPSK500 ||
		numcarriers > 1 ) return;
//Android: re-enable (delete return below and check for multi-carrier modes) when have selectable afc
	 return;
...
void psk::afc()
{
//Android: re-enable (un-comment below) when have selectable afc
//Android Always OFF if (!progStatus.afconoff)
		return;
	if (dcd == true || acquire)
		phaseafc();
}
...
void psk::rx_symbol(cmplx symbol, int car)
{
	...
	double softangle;
	double softamp;
//Android using older complex.h header	double sigamp = norm(symbol);
	double sigamp = symbol.norm();

	static double averageamp;

//Android using older complex.h header	phase = arg ( conj(prevsymbol[car]) * symbol );
...
//Android	metric = 100.0 * norm(quality);
	metric = 100.0 * quality.norm();

//Android yes always
//Android	if (progdefaults.Pskmails2nreport && (mailserver || mailclient)) {
		//s2n reporting: rescale depending on mode, clip after scaling
		if (_pskr)
			s2n_metric = metric * 1.5 + 8;
		else
			s2n_metric = metric;
		//Android 	s2n_metric = CLAMP(s2n_metric, 0.0, 100.0);
...
//Android old complex.h	afcmetric = decayavg(afcmetric, norm(quality), 50);
	...
	switch (dcdshreg) {
		...
		default:
//Android		if (metric > progStatus.sldrSquelchValue || progStatus.sqlonoff == false) {
...
//Android
	...
	if ( 1 == set_dcdON ) {
		...
		imdValid = true;
//Android		if (progdefaults.Pskmails2nreport && (mailserver || mailclient))
//Android			s2n_sum = s2n_sum2 = s2n_ncount = 0.0;
		//printf("\n DCD ON!!");
	} else if ( 0 == set_dcdON ){
		dcd = false;
		acquire = 0;
	...
	if (_pskr) {
		rx_pskr(softbit);
		//Android No digiscope		set_phase(phase, norm(quality), dcd);
...
//Android short buffer now
//int psk::rx_process(const double *buf, int len)
//int psk::rx_process(const short *buf, int len)
...
//Android from double to shorts			z = cmplx ( *buf * cos(phaseacc[car]), *buf * sin(phaseacc[car]) );
			z = cmplx ( (double)(*buf) * cos(phaseacc[car]), (double)(*buf) * sin(phaseacc[car]) );
...
//			if (vestigial && car == 0) vestigial_sfft->run(z, sfft_bins, 1);
	...
	*/
					...
					for (int ii = 0; ii < numcarriers; ii++) {
						//Android old complex.h				sum += abs(z2[ii])/numcarriers;
						sum += sqrt(z2[ii].norm())/numcarriers;
					}
	...
	*/
	...
	//Android do nothing (not required for flmsg app)
	/*	if (sigsearch)
		findsignal();
	else if (mailserver) {
		if (waitcount > 0) {
