...
bool mfsk::check_picture_header(char c)
{
	...
	if (!*p)
		return false;
	RXspp = 8;
	//Test narrower modes/slower speeds
	if (*p == '6') RXspp = 32;
	if (*p == '5') RXspp = 16;
	if (*p == '4') RXspp = 4;
	if (*p == '2') RXspp = 2;
	//Android: For sending in wide modes like MFSK128 over good FM links in particular
	if (*p == '1') RXspp = 1;
	p++;
	if (!*p)
		return false;
...
//Android	picf += arg( conj(prevz) * z) * samplerate / TWOPI;
	...
	if ((counter % RXspp) == 0) {
	    //Minimize whitening out of pixels in case of frequency differences
		picf = 256 * (picf / RXspp - basefreq) / bandwidth;
		//picf = 253 * (picf / RXspp - basefreq) / bandwidth;
		...
		if (color) {
			...
			if (++col == picW) {
				...
				if (++rgb == 3) {
					rgb = 0;
					row += 3 * picW;
					//rxPercent = (100 * row) / (picH * 3);
				}
			}
		} else {
			for (int i = 0; i < 3; i++) {
				//Android: process three identical rows at a time
				pixelnbr = i + 3*col;
				//Android  REQ(updateRxPic, byte, pixelnbr++);
				androidUpdateRxPic(byte, pixelnbr);
			}
...
//JD Not yet				put_status("Tx pic: start");
                // VK2ETA send extra prologue for MFSK128
                if (mode == MODE_MFSK128) {
                    for (int i = 0; i < 12; i++)
                        sendchar('\r');
                 }
